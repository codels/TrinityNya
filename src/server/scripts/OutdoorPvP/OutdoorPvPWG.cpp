...
bool OutdoorPvPWG::SetupOutdoorPvP()
{
    ...
    if (!result)
        sLog->outError(LOG_FILTER_GENERAL, "Cannot find siege workshop master or spirit guides in creature!");
    else
    {
        do
        {
    ...
    do
    {
        ...
        if (capturePointEntry)
        {
            ...
            if (!engGuid)
            {
                sLog->outError(LOG_FILTER_GENERAL, "Cannot find nearby siege workshop master!");
                continue;
            }
            else
            ...
            {
                delete workshop;
                sLog->outError(LOG_FILTER_GENERAL, "Cannot add capture point!");
                continue;
            }
    ...
    if (!m_gate)
    {
        sLog->outError(LOG_FILTER_GENERAL, "Cannot find wintergrasp fortress gate!");
        return false;
    }
...
void OutdoorPvPWG::ModifyWorkshopCount(TeamId team, bool add)
{
    ...
    if (add)
        ...
        sLog->outError(LOG_FILTER_GENERAL, "OutdoorPvPWG::ModifyWorkshopCount: negative workshop count!");
