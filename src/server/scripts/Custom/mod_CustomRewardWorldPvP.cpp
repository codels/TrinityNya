...
class Mod_CustomRewardPvP_WorldScript : public WorldScript
{
    ...
    void LoadDataFromDataBase()
    {
        ...
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading CustomRewardPvPInfo...");
        ...
        if (!result)
        {
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> `world_custom_reward_pvp` is empty");
            return;
        }
        ...
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u count for CustomRewardPvP in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
